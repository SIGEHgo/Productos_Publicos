...
function agregarCatalogoCobPot(capas, groupTitle, folder) {
    ...
    const byOrden = {};
    for (const key in capas.slice(0,5)) {
        const capa = capas[key];
...
async function cargarDiccionariosAplicacion() {
    try {
        const [diccionarioAvistamientos, datosTaxonomia, diccionarioRiesgo, datosTaxonomiaSigeh, catalogoCapasInfo, catalogoAnfibios, catalogoAves,busquedaInv] = await Promise.all([
            ...
            cargarDiccionarioJson("assets/avistamientos/busqueda_inversa.json"),
        ]);
        ...
        // Construir busqueda_lista desde busqueda_inversa.json (ya incluye n, reino, orden, familia, nombre_cientifico)
        // Deduplicar por nombre_comun y sumar n para nombres repetidos
        const seen = new Map();
        for (const item of busquedaInv) {
            if (!item.nombre_comun) continue;
            if (seen.has(item.nombre_comun)) {
                seen.get(item.nombre_comun).n += item.n || 0;
            } else {
                seen.set(item.nombre_comun, { ...item });
            }
        }
        busqueda_lista = Array.from(seen.values());
